...
namespace Brambles {
    ...
    private:
        ...
        void handleCollision(std::shared_ptr<BoxCollider> collider1,
                           std::shared_ptr<BoxCollider> collider2);
        void handleBoxMeshCollision(std::shared_ptr<BoxCollider> box,
                                  std::shared_ptr<MeshCollider> mesh);

        bool checkBoxMeshCollision(std::shared_ptr<BoxCollider> box,
                                 std::shared_ptr<MeshCollider> mesh);
        bool triangleAABBOverlap(const glm::vec3& v0, const glm::vec3& v1, const glm::vec3& v2,
